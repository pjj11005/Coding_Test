        ...
        for j in range(n):
            for di, dj in check:
                ...
                if 0 <= ni < m and 0 <= nj < n and visited[i][j] != visited[ni][nj]:
                    answer = max(
                        answer, roomsize[visited[i][j]] + roomsize[visited[ni][nj]]
                    )
...
array = [
    list(map(bin, map(int, input().split()))) for _ in range(m)
]  # 이진수로 변환하여 입력 받음
...
for i in range(m):
    for j in range(n):
        array[i][j] = list(array[i][j][2:].zfill(4))
